...
void weather_layer_init(WeatherLayer* weather_layer, GPoint pos) {
	...
	// Add top layer
	text_layer_init(&weather_layer->temp_layer_top, GRect(0, 10, 144, 9));
	text_layer_set_background_color(&weather_layer->temp_layer_top, GColorWhite);
	layer_add_child(&weather_layer->layer, &weather_layer->temp_layer_top.layer);
	// Add temperature layer
	text_layer_init(&weather_layer->temp_layer, GRect(0, 18, 144, 80));
...
void weather_layer_set_icon(WeatherLayer* weather_layer, WeatherIcon icon) {
	...
	layer_set_frame(&weather_layer->icon_layer.layer.layer, GRect(6, 8, 70, 70));
